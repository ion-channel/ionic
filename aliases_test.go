...
func TestAliases(t *testing.T) {
	...
	g.Describe("Alias", func() {
		...
		g.It("should add an alias", func() {
			...
			project, err := client.AddAlias("f9bca953-80ac-46c4-b195-d37f3bc4f498", "ateamid", "name", "version", "token")
			Expect(err).To(BeNil())
			Expect(project.ID).To(Equal("334c183d-4d37-4515-84c4-0d0ed0fb8db0"))
			Expect(project.Type).To(Equal("git"))
			Expect(len(project.Aliases)).To(Equal(1))
		})
