...
func TestReports(t *testing.T) {
	...
	g.Describe("Reports", func() {
		...
		g.It("should get exported projects data", func() {
			server.AddPath("/v1/report/getExportedData").
				SetMethods("POST").
				SetPayload([]byte(SampleExportedProjectsData)).
				SetStatus(http.StatusOK)

			d, err := client.GetExportedProjectsData([]string{"foo_id", "bar_id"}, "someTeamID", "atoken")
			Expect(err).To(BeNil())

			Expect(d.Projects).NotTo(BeNil())
			Expect(d.Projects[0]).NotTo(BeNil())

			Expect(d.Projects[0].ProjectID).To(Equal("foo_id"))
			Expect(d.Projects[0].ProjectName).To(Equal("foo"))
			Expect(d.Projects[0].ProductName).To(Equal("baz"))
			Expect(d.Projects[0].Org).To(Equal("org/foo"))
			Expect(d.Projects[0].Version).To(Equal("2"))
			...
			Expect(d.Projects[1].ProductName).To(Equal(""))
			Expect(d.Projects[1].Org).To(Equal(""))
			Expect(d.Projects[1].Version).To(Equal(""))
			...
			Expect(*d.Projects[1].VirusCount).To(Equal(1))
		})
	})
}
...
const (
	...
	SampleExportedProjectsData = `{"data":{"created_at":"2020-10-14T18:44:36.1740216Z","projects":[{"project_name":"foo","project_id":"foo_id","product_name":"baz","version":"2","org":"org/foo","current_status":"failed","vuln_count":23,"critical_vuln_count":1,"high_vuln_count":5,"virus_count":0},{"project_name":"bar","project_id":"bar_id","product_name":"","version":"","org":"","current_status":"failed","vuln_count":26,"critical_vuln_count":3,"high_vuln_count":9,"virus_count":1}]},"meta":{"total_count":2,"offset":0,"last_update":"2020-10-14T18:44:36.1740346Z"}}`
)
