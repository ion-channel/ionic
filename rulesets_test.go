...
func TestRuleSets(t *testing.T) {
	...
	g.Describe("RuleSets", func() {
		...
		g.It("should get a project history", func() {
			server.AddPath("/v1/ruleset/getProjectHistory").
				SetMethods("GET").
			...
			Expect(ph[0].AnalysisID).To(Equal("8f43ffbc-672e-42b4-b1a5-e69b2a5d0b8e"))
			Expect(ph[1].Status).To(Equal(false))
		})

		g.It("should get a ruleset names", func() {
			server.AddPath("/v1/ruleset/getRulesetNames").
				SetMethods("POST").
				SetPayload([]byte(SampleRulesetNames)).
				SetStatus(http.StatusOK)

			rn, err := client.GetRulesetNames([]string{"276bbec3-cc77-44b9-a46d-c7760947ec9d", "B061D58B-FDFD-46BF-A766-2D38DE3B1D7B"}, "sometoken")
			Expect(err).To(BeNil())
			Expect(len(rn)).To(Equal(2))
			Expect(rn[0].ID).To(Equal("276bbec3-cc77-44b9-a46d-c7760947ec9d"))
			Expect(rn[0].Name).To(Equal("ruleset1"))
			Expect(rn[1].ID).To(Equal("B061D58B-FDFD-46BF-A766-2D38DE3B1D7B"))
			Expect(rn[1].Name).To(Equal("ruleset2"))
		})

		g.It("should get analyses statuses", func() {
			server.AddPath("/v1/ruleset/getAnalysesStatuses").
				SetMethods("POST").
				SetPayload([]byte(SampleAnalysesStatuses)).
				SetStatus(http.StatusOK)

			statuses, err := client.GetAnalysesStatuses("ateamid", []string{"analysis_id1", "analysis_id2"}, "")
			Expect(err).To(BeNil())
			Expect(len(statuses)).To(Equal(2))
			Expect(statuses[0].AnalysisID).To(Equal("analysis_id1"))
			Expect(statuses[0].ProjectID).To(Equal("project_id1"))
			Expect(statuses[0].Status).To(Equal("pass"))
			Expect(statuses[1].AnalysisID).To(Equal("analysis_id2"))
			Expect(statuses[1].ProjectID).To(Equal("project_id2"))
			Expect(statuses[1].Status).To(Equal("fail"))
		})
	})
}
...
const (
	...
	SampleProjectHistory   = `{"data":[{"team_id":"276bbec3-cc77-44b9-a46d-c7760947ec9d","project_id":"c0210380-3d44-495d-9d10-c7d436a63870","analysis_id":"8f43ffbc-672e-42b4-b1a5-e69b2a5d0b8e","pass":true,"created_at":"2020-06-17T23:11:18.435151Z"},{"team_id":"276bbec3-cc77-44b9-a46d-c7760947ec9d","project_id":"c0210380-3d44-495d-9d10-c7d436a63870","analysis_id":"00be1862-959c-45d8-8fb5-2b748fe854d6","pass":false,"created_at":"2020-06-16T23:11:18.435151Z"}],"meta":{"total_count":2,"offset":0,"last_update":"0001-01-01T00:00:00Z"}}`
	SampleRulesetNames     = `{"data":[{"id":"276bbec3-cc77-44b9-a46d-c7760947ec9d","name":"ruleset1"},{"id":"B061D58B-FDFD-46BF-A766-2D38DE3B1D7B","name":"ruleset2"}],"meta":{"total_count":2,"offset":0,"last_update":"0001-01-01T00:00:00Z"}}`
	SampleAnalysesStatuses = `{"data":[{"analysis_id":"analysis_id1","project_id":"project_id1","status":"pass"},{"analysis_id":"analysis_id2","project_id":"project_id2","status":"fail"}],"meta":{"total_count":2,"offset":0,"last_update":"0001-01-01T00:00:00Z"}}`
)
