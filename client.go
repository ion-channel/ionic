...
func (ic *IonClient) _do(method, endpoint, token string, params *url.Values, payload bytes.Buffer, headers http.Header, page *pagination.Pagination) (*IonResponse, *errors.IonError) {
	...
	if err != nil {
		return nil, errors.Errorf("no body", 0, "failed to create request: %v", err.Error())
	}
	...
	if err != nil {
		return nil, errors.Errorf("no body", 0, "failed http request: %v", err.Error())
	}
	defer resp.Body.Close()

	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return nil, errors.Errorf("no body", resp.StatusCode, "failed to read response body: %v", err.Error())
	}

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, errors.Errorf(string(body), resp.StatusCode, "error response from API")
	}

	var ir IonResponse
	err = json.Unmarshal(body, &ir)
	if err != nil {
		return nil, errors.Errorf(string(body), resp.StatusCode, "malformed response: %v", err.Error())
	}
