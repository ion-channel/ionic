...
func TestScanResults(t *testing.T) {
	...
	g.Describe("Scan Results", func() {
		...
		g.It("should unmarshal a scan results with difference data", func() {
			...
			Expect(ok).To(Equal(true))
			Expect(d.Checksum).To(Equal("checksumhere"))
		...
		g.It("should unmarshal a scan results with external vulnerabilities scan data", func() {
			...
			Expect(err).To(BeNil())
			Expect(r.Type).To(Equal("external_vulnerabilities"))
...
const (
	...
	SampleValidExternalVulnerabilities  = `{"type":"vulnerability","data":{"critical":1,"high":0,"medium":1,"low": 0}}`
)
