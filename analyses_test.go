...
func TestAnalysis(t *testing.T) {
	...
	g.Describe("Analysis", func() {
		...
		g.It("should get the latest analysis summaries", func() {
			server.AddPath("/v1/animal/getLatestAnalysisSummaries").
				SetMethods("POST").
			...
			Expect(analyses[0].Passed).To(Equal(false))
		})
		...
		g.It("should return analyses exported data", func() {
			server.AddPath("/v1/animal/getAnalysesExportData").
				SetMethods("POST").
				SetPayload([]byte(SampleAnalysesExport)).
				SetStatus(http.StatusOK)

			r, err := client.GetAnalysesExportData("cuketest", []string{"2e85e255-a558-4217-8a5b-98cb4c26d908", "ae7a2df7-f76f-4f79-9e5f-50916ec2a787"}, "sometoken")
			Expect(err).To(BeNil())
			Expect(len(r)).To(Equal(2))
			Expect(r[0].AnalysisID).To(Equal("2e85e255-a558-4217-8a5b-98cb4c26d908"))
			Expect(r[0].ProjectID).To(Equal("48ab4bf9-a567-47f5-b481-b1cc771aee02"))
			Expect(r[0].Status).To(Equal("finished"))
			Expect(r[0].VirusCount).To(Equal(1))
			Expect(r[0].VulnerabilityCount).To(Equal(1))
			Expect(r[0].HighVulnCount).To(Equal(1))
			Expect(r[0].CritVulnCount).To(Equal(0))

			Expect(r[1].AnalysisID).To(Equal("ae7a2df7-f76f-4f79-9e5f-50916ec2a787"))
			Expect(r[1].ProjectID).To(Equal("11597c1d-8a46-4020-b294-41ddab10fb1b"))
			Expect(r[1].Status).To(Equal("finished"))
			Expect(r[1].VirusCount).To(Equal(0))
			Expect(r[1].VulnerabilityCount).To(Equal(6))
			Expect(r[1].HighVulnCount).To(Equal(3))
			Expect(r[1].CritVulnCount).To(Equal(2))
		})
	})
